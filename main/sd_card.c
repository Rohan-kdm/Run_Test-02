...
esp_err_t waveshare_sd_card_init()
{
    ...
    /* EXIO4 is active low. Select SD without changing EXIO2/backlight. */
    ESP_RETURN_ON_ERROR(ch422g_set_sd_selected(true), TAG, "Select SD card");
    ...
    ESP_LOGI(TAG, "SPI Host = %d", host.slot);

    ESP_LOGI(TAG,
            "SPI Pins: MOSI=%d MISO=%d CLK=%d CS=%d",
            PIN_NUM_MOSI,
            PIN_NUM_MISO,
            PIN_NUM_CLK,
            PIN_NUM_CS);
