...
#include "waveshare_rgb_lcd_port.h"
#include "ch422g.h" // Added for thread-safe I/O expander access

// Define the CH422G bitmasks based on the original raw I2C writes
// #ifndef CH422G_IO_LCD_BL
// #define CH422G_IO_LCD_BL    (1 << 2) // Deduced from 0x1E (ON) vs 0x1A (OFF)
// #endif

#ifndef CH422G_IO_TOUCH_RST
#define CH422G_IO_TOUCH_RST (1 << 1) // Deduced from 0x2C (LOW) vs 0x2E (HIGH)
#endif
...
{
    /* 
     * FIX: The I2C driver is already installed during the SD Card initialization.
     * Attempting to configure and install it again here causes an ESP_ERR_INVALID_STATE.
     * We bypass the installation to share the existing I2C bus safely.
     */
...
{
    /* 
     * FIX: Replaced raw i2c_master_write_to_device() with thread-safe CH422G API.
     * This prevents clobbering the SD Card CS pin (Bit 4) while resetting the touch panel.
     */
    
    // Pull the touch reset pin LOW (Previously raw write 0x2C)
    ...
    // Pull the touch reset pin HIGH (Previously raw write 0x2E)
...
esp_err_t waveshare_esp32_s3_rgb_lcd_init()
{
    ...
    gpio_init(); // Initialize GPIO pins
    ESP_LOGI(TAG, "Initialize Touch LCD"); // Log touch LCD initialization
    waveshare_esp32_s3_touch_reset(); // Reset the touch panel (Now thread-safe)
...
esp_err_t wavesahre_rgb_lcd_bl_on()
{
    /* 
     * FIX: Replaced raw i2c_master_write_to_device() with thread-safe CH422G API.
     * Pulls the backlight pin high without overwriting the SD Card CS pin.
     */
    // ch422g_set_bit(CH422G_IO_LCD_BL);
    // return ESP_OK;
...
esp_err_t wavesahre_rgb_lcd_bl_off()
{
    /* 
     * FIX: Replaced raw i2c_master_write_to_device() with thread-safe CH422G API.
     * Pulls the backlight pin low without overwriting the SD Card CS pin.
     */
    // ch422g_clear_bit(CH422G_IO_LCD_BL);
    // return ESP_OK;
