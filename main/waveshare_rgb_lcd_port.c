...
void gpio_init(void)
{
    ...
    // GPIO4 is GT911 TP_IRQ; it must never be driven by the ESP32.
...
void waveshare_esp32_s3_touch_reset()
{
    ...
    ESP_ERROR_CHECK(ch422g_clear_bit(CH422G_IO_TOUCH_RST));
    esp_rom_delay_us(100 * 1000);
    gpio_set_level(GPIO_INPUT_IO_4, 0);
    esp_rom_delay_us(100 * 1000);
    ESP_ERROR_CHECK(ch422g_set_bit(CH422G_IO_TOUCH_RST));
...
esp_err_t waveshare_esp32_s3_rgb_lcd_init()
{
    ...
    ESP_RETURN_ON_ERROR(ch422g_init(), TAG, "Initialize CH422G");
...
{
    return ch422g_set_backlight(true);
}

/******************************* Turn off the screen backlight **************************************/
esp_err_t wavesahre_rgb_lcd_bl_off()
{
    return ch422g_set_backlight(false);
}
